...
public class FilterFramework extends Thread
{
	...
	private ArrayList<PipedInputStream> InputReadPorts = new ArrayList<>();
	...
	private ArrayList<FilterFramework> InputFilter = new ArrayList<>();
	...
	****************************************************************************/
	...
	void Connect( FilterFramework Filter )
	{
		try
		{
			...
			PipedInputStream tmp = new PipedInputStream();	// criamos um pipe de input e adicionamos ao arraylist
			tmp.connect( Filter.OutputWritePort );			// para conseguirmos ler de varios filtros ao mesmo tempo
			InputReadPorts.add(tmp);
	...
	****************************************************************************/
	...
	byte ReadFilterInputPort(int x) throws EndOfStreamException
	{
		byte datum = 0;
		...
		try
		{
			while (InputReadPorts.get(x).available()==0 )
			{
				if (EndOfInputStream(x))
				{
		...
		try
		{
			datum = (byte)InputReadPorts.get(x).read();
			return datum;
	...
	****************************************************************************/
	...
	private boolean EndOfInputStream(int x)
	{
		if (InputFilter.get(x).isAlive())
		{
			return false;
	...
	****************************************************************************/
	...
	void ClosePorts()
	{
		try
		{
			for(PipedInputStream p: InputReadPorts){
				p.close();
			}
